...
terraform:register_tool("brush", {
	...
	modifiers = {{n="surface",l="Surface"}, {n="place_on_surface",l="Place On Surface"}, {n="scatter",l="Scatter"}, {n="decor",l="Decoration"}, {n="landslide",l="Landslide"}, {n="flat", l="Flat"}},
...
-- НОВИЙ МОДИФІКАТОР: розміщення на поверхні
if settings:get_int("modifiers_place_on_surface") == 1 then
    table.insert(modifiers, function(i)
        if data[i] ~= minetest.CONTENT_AIR then 
            -- Якщо поточний блок не повітря, шукаємо наступний блок вгору
            local current_i = i
            while data[current_i + a.ystride] ~= minetest.CONTENT_AIR and a:position(current_i).y < maxp.y do
                current_i = current_i + a.ystride
            end
            -- Перевіряємо чи наступний блок - повітря (місце для розміщення)
            if data[current_i + a.ystride] == minetest.CONTENT_AIR then
                return current_i + a.ystride
            end
            return nil
        end
        return i
