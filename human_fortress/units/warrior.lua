...
mobs:register_mob("human_fortress:warrior", {
    ...
    -- Анімації (якщо твоя модель підтримує ті ж імена, що й у робітника)
    animation = {
        speed_normal = 1,
        speed_run = 15,
        stand_start = 13,
    ...
    on_activate = function(self, staticdata, dtime_s)
        ...
        -- Забираємо команду з черги, якщо юніт прокинувся
    ...
    on_die = function(self, pos)
        ...
        minetest.add_item(drop_pos, "human_fortress:versiforn_source 40")
        minetest.add_item(drop_pos, "human_fortress:ether_tree 40")

        local owner = self.unit_data and self.unit_data.owner or ""
        if owner ~= "" then
            minetest.chat_send_player(owner, "💀 Твій Воїн загинув у бою!")
        end
    ...
    do_custom = function(self, dtime)
        ...
        -- Допоміжна функція руху (копія з worker)
        ...
        if self.unit_data.command then
            ...
            if owner ~= "" then
                minetest.chat_send_player(owner, "⚔️ Воїн отримав наказ: " .. (self.order or "очікування"))
            end
        end
            ...
            if target_obj and target_obj:get_pos() then
                ...
                if dist > 2 then
                    ...
                    target_obj:punch(self.object, 1.0, {
                        ...
                        damage_groups = {fleshy = warrior_data.damage},
                    }, nil)
...
function human_fortress.spawn_warrior(pos, player_name)
    local ent = minetest.add_entity(pos, "human_fortress:warrior")
...
print("[Human Fortress] Юніт 'warrior' зареєстровано (RTS-версія, стійка до вивантаження чанків)")